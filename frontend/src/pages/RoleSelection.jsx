          ...
          >
            ...
            <div className="sticky top-0 flex items-center justify-between px-6 py-4 bg-[#141210]/95 backdrop-blur-md border-b border-white/[0.06]">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-bold text-white text-lg">参赛协议与安全声明</h3>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="px-6 py-6 space-y-8 text-sm leading-relaxed text-slate-300">
              ...
              <section>
                ...
                <div className="space-y-2 text-slate-400">
                  <p>· 所有赛道不限身份、不限年龄、不限技术背景，线上提交，无需到场。</p>
                  <p>· 参赛者须提交：PPT（10页以内）+ 演示视频（3分钟以内），海报与在线链接为可选项。</p>
                  <p>· 每位参赛者每个赛道限提交一份作品，每个邮箱仅可报名一次。</p>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between py-2 border-b border-white/[0.05] last:border-0">
                      ...
                      <span className="font-mono text-primary text-xs">{t.date}</span>
                    </div>
                  ))}
                </div>
              </section>
              ...
              <section>
                ...
                <p className="text-slate-500 mb-3">本赛事倡导负责任的 AI Agent 使用，参赛作品须遵守以下原则：</p>
              ...
              <section>
                ...
                <div className="space-y-2 text-slate-400">
                  <p>· 参赛者提交的个人信息（姓名、邮箱、机构）仅用于赛事通知和奖项发放，不会对外出售或滥用。</p>
                  <p>· 参赛者授权组委会在赛事宣传材料中展示项目标题、演示内容及获奖信息。</p>
              ...
              <section className="pt-2 border-t border-white/[0.06]">
                <p className="text-slate-500 text-xs">
                  <span className="text-slate-400 font-medium">主办：</span>北京中关村学院 · 中关村人工智能研究院 · AI商学院
                </p>
                <p className="text-slate-500 text-xs mt-1">
                  <span className="text-slate-400 font-medium">赞助：</span>北京中关村学院教育基金会 · 海淀区西北旺政府
                </p>
              </section>
            </div>
          </motion.div>
...
function TiltCard({ role, onClick, index }) {
  ...
  return (
    <motion.div
      ...
      transition={{ delay: 0.3 + index * 0.12, duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
      ...
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-4">
            <div className={`size-12 rounded-xl ${iconBg} flex items-center justify-center flex-shrink-0 transition-all duration-300 border border-current/10`}>
              <span className={`material-symbols-outlined text-2xl ${iconColor}`}>{role.icon}</span>
            </div>
            <p className={`text-xs ${subtitleColor} font-bold uppercase tracking-[0.2em]`}>{role.subtitle}</p>
          </div>
          <h3 className="text-2xl font-black mb-2 text-white tracking-tight">{role.title}</h3>
          <p className="text-slate-400 leading-relaxed text-sm mb-5">{role.description}</p>
          ...
          <div className={`flex items-center gap-2 ${subtitleColor} font-bold text-sm`}>
            <span>立即进入</span>
            <motion.span
...
export default function RoleSelection() {
  ...
  const roles = [
    {
      ...
      description: "展示你的龙虾能干什么——学术、生产力或生活三大赛道，无论背景皆可参赛",
      icon: "cruelty_free",
      features: [
        "选择赛道并填写个人与项目信息",
        "上传 PPT 材料与演示视频链接",
  ...
  const timeline = [
    { phase: "报名 + 提交", date: "3月11日—19日", icon: "edit_note", active: true },
    { phase: "线上评审", date: "3月20日—21日", icon: "fact_check", active: false },
  ...
  return (
    <>
      ...
      {showWelcome && (
        <WelcomeAnimation onDone={() => setShowWelcome(false)} />
      )}
      ...
      <div className="h-screen overflow-hidden bg-[#0c0a09] font-display text-slate-100 relative flex flex-col">
        ...
        <div className="absolute inset-0 z-10 pointer-events-none"><FloatingSymbols /></div>
        ...
        <header className="relative z-20 flex-shrink-0 flex items-center justify-between px-6 py-4 lg:px-16 border-b border-white/[0.06] backdrop-blur-md bg-black/20">
          ...
          >
            ...
            <div className="leading-tight">
              ...
              <span className="text-[10px] text-slate-600 ml-2 font-mono">第一届</span>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-2 px-4 py-1.5 bg-primary/8 rounded-full border border-primary/15 backdrop-blur-sm">
              ...
              <span className="text-xs text-primary font-medium">报名通道开放中</span>
            </div>
            {daysLeft > 0 && (
        ...
        {/* ── Main ── */}
        <main className="relative z-20 flex-1 flex flex-col lg:flex-row items-center justify-center px-6 lg:px-16 gap-8 lg:gap-16 min-h-0">
          {/* Left: title + info */}
          <motion.div
            initial={{ opacity: 0, x: -30 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.65, ease: [0.22, 1, 0.36, 1] }}
            className="flex flex-col justify-center lg:w-[42%] flex-shrink-0"
          >
            {/* Tagline badge */}
            <motion.div
            ...
            >
              <span className="text-[10px] font-mono text-primary/80 tracking-[0.2em] uppercase">全网首个官方龙虾赛事</span>
            </motion.div>

            {/* Title */}
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black tracking-tight leading-[1.0] mb-3">
            ...
            <div className="flex items-center gap-6 text-center mb-6">
              {[
                ...
                >
                  <span className="text-xl font-black text-white/80">{stat.num}</span>
              ...
              {timeline.map((t, i) => (
                <div key={i} className="flex items-center">
                  <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-white/[0.03] border border-white/[0.06]">
                    <span className={`material-symbols-outlined text-xs ${t.active ? "text-primary" : "text-slate-600"}`}>
                      {t.icon}
                    </span>
                    <div>
                      <p className={`text-[9px] font-bold ${t.active ? "text-primary" : "text-slate-500"}`}>{t.phase}</p>
                      <p className="text-[9px] text-slate-600 font-mono">{t.date}</p>
                    </div>
                  </div>
                  {i < timeline.length - 1 && (
            ...
            >
              <div>
                <p className="text-[10px] text-slate-600">
                  主办：<span className="text-slate-500">北京中关村学院 · 中关村人工智能研究院 · AI商学院</span>
                </p>
                <p className="text-[10px] text-slate-600 mt-0.5">
                  赞助：<span className="text-slate-500">北京中关村学院教育基金会 · 海淀区西北旺政府</span>
                </p>
              </div>
              <button
                onClick={() => setTermsOpen(true)}
                className="flex items-center gap-1 text-[10px] text-slate-600 hover:text-primary transition-colors flex-shrink-0 ml-4"
              >
          ...
          {/* Right: role cards */}
          <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-[58%] max-w-3xl">
            {roles.map((role, i) => (
              <TiltCard
                key={role.id}
                role={role}
        ...
        {/* ── Footer ── */}
        <footer className="relative z-20 flex-shrink-0 border-t border-white/[0.05] py-3 px-6 flex items-center justify-between">
